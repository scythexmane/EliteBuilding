// AboutPage.jsx – «О нас» с центральной зиг‑линией и финальной кнопкой
// Линия держится строго по центру секции, амплитуда увеличена,
// добавлена кнопка «На главную» в конце страницы, с поддержкой мультиязычности
...
const TimelineLine = React.forwardRef((_, ref) => (
  <svg
    className="absolute left-1/2 -translate-x-1/2 top-0 h-full w-[80px] hidden md:block"
    viewBox="0 0 60 2000"
...
export default function AboutPage() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const end = winH + height;
      const progress = Math.min(
        Math.max((winH - top - start) / (end - start), 0),
        1
      );
  ...
  return (
    <main className="bg-gray-950 text-white">
      <section
        className="relative pt-36 pb-20 px-4 text-center"
        style={{
          backgroundImage:
            "url('https://wallpapers.com/images/hd/blue-marble-1920-x-1080-wallpaper-zz4u51hcokbczc5v.jpg')",
      ...
      >
        ...
        <div className="relative z-10 max-w-6xl mx-auto grid lg:grid-cols-2 gap-14">
          <div
      ...
      <section
        ref={sectionRef}
        className="relative py-32 bg-[#08122F] text-amber-400 overflow-hidden"
      >
        <TimelineLine ref={pathRef} />
        <div className="relative z-10 container mx-auto px-6 md:px-12 space-y-56">
          {timeline.map((e, idx) => (
            <div
              key={e.year}
            ...
            >
              ...
              >
                <div className="transform transition-transform duration-700 hover:scale-[1.04] rotate-2 -rotate-[6deg] md:rotate-[6deg]">
                  <img
                    src={e.image}
                    alt={e.year}
                    className="w-full rounded-3xl shadow-xl rotate-[6deg] md:rotate-[-6deg]"
                  />
                </div>
              </div>
              <div className="w-full md:w-1/2 text-center md:text-left space-y-4">
                ...
                <div className="h-1 w-14 mx-auto md:mx-0 bg-amber-400 rounded-full" />
